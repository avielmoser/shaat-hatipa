...
export async function getDashboardMetrics(prisma: PrismaClient, filters: FilterParams): Promise<DashboardMetrics> {
    ...
    // Clinic Filter
    const clinicFilter = clinicSlug ? { clinicSlug } : {};

    // Protocol Filter (requires inspecting meta for surgeryType/protocolKey)
    // Prisma JSON filtering: meta: { path: ['surgeryType'], equals: protocol }
    ...
    // 1. Fetch RAW events needed for calculation
    // We fetch minimal fields to perform memory aggregation for complex KPIs (like time diffs)
    // For simple counts, we can use count() queries but one big fetch might be more consistent for session-based logic.
    // However, fetching all events for a year is heavy.
    // Let's use specific aggregations where possible, and fetch raw for session flows.
